...
- Direct dependencies: Node 20+, TypeScript/Bun, raw `@modelcontextprotocol/sdk`, Vercel AI SDK v6 (`ai` package), `@openrouter/ai-sdk-provider`, Playwright the library, `ffmpeg-static`, react-grab (transitively pulls bippy). Runtime child spawn: Playwright MCP. OpenRouter routes to `qwen/qwen3.6-flash` for the v0.1 raw-video VLM path; deterministic retrieval answers from local artifacts without GPT/OpenAI in the primary path.
