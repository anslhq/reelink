...
The system SHALL expose BugPack through a small local `bugpack_*` MCP tool surface suitable for coding/browser agents.

#### Scenario: MCP server launched
- **WHEN** an agent launches BugPack with `npx -y bugpack`
- **THEN** the system SHALL start a Node 20+ TypeScript stdio MCP server by default
- **AND** the server SHALL expose a small set of `bugpack_*` tools with path-oriented returns

#### Scenario: Tool surface constrained
- **WHEN** the MCP server is available
- **THEN** it SHALL include the core tools `bugpack_analyze`, `bugpack_get_finding`, `bugpack_get_frame`, `bugpack_get_dom`, `bugpack_get_components`, `bugpack_query`, and `bugpack_run` as implemented by layer
...
- **WHEN** a user runs `npx bugpack init`
- **THEN** the system SHALL create or update non-secret config at `~/.bugpack/config.json`
...
- **WHEN** a user runs `bugpack doctor`
- **THEN** the system SHALL check Node version, BugPack package resolution, Playwright browser install, MCP config, hosted model key presence, and LM Studio local fallback availability
- **AND** it SHALL report actionable fixes without printing API key values

### Requirement: Agent Self-Recording Workflow
The system SHALL provide `bugpack_run(task_description, target_url)` so agents can record their own browser tasks and reuse the recording as state.

#### Scenario: Agent run succeeds
- **WHEN** an agent calls `bugpack_run` with a task description and target URL
